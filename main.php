<?php
	...
	function getport($school)
	{
		...
		else if	($school==25)	return 10021	;//二分
	...
	function chaxun($sno,$ps=1000,$pn=1,$port=0)
	{
		...
		else
		{
			...
			foreach($card->data->trades as $trade)
				$data->logs[]=(object)array('time'=>$trade->time,'consumtype'=>$trade->consumType,'amount'=>(float)($trade->amount),'type'=>$trade->type,'mercname'=>$trade->mercName==''?'N/A':$trade->mercName,'mercaccount'=>$trade->mercAccount);
		}
		return $data;
	}
