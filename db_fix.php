<?php
	...
	foreach($data as $one)
	{
		...
		if($stu->code)
		{
			db_insert_student($conn,$stu);echo '学生'.$stu->xjh."\t姓名:".$stu->name."\t卡号:".$stu->card_id."\t余额:".$stu->amount."\t";
	//		db_insert_extern($conn,($ex=get_student_extern($one['xjh'],getport($one['school']))));echo "银行卡号:".$ex->bankcard."\t身份证号:".$ex->china_id_card."\t性别:".($ex->sex?'男':'女')."\t生日:".$ex->birthday;
			...
			while($buff>=$ps)
			{
				$delta_log+=$buff=db_insert_logs($conn,$one['xjh'],($logs=get_student_logs($one['xjh'],getport($one['school']),$ps,$pn)),true,$one['check_point']);
...
//			$st->execute();			
			...
			$st->bindValue(1,$one['xjh']);
			$st->execute();		
			$st=$conn->prepare("UPDATE `qiafan`.`student` SET `qiafan`.`student`.`amount_abs`=ROUND(IFNULL((SELECT SUM(ABS(`qiafan`.`logs`.`amount`)) FROM `qiafan`.`logs` WHERE `qiafan`.`logs`.`xjh`=`qiafan`.`student`.`xjh`),0),2) WHERE `xjh`=?;");
			$st->bindValue(1,$one['xjh']);
			$st->execute();
		}
		$i++;
